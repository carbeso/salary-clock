...
6. **100% 隱私安全 (LocalStorage + Cookie 雙重本機保存)**
   - 純本機運作，所有薪資設定皆同步保存於瀏覽器的 `LocalStorage` 與 `Cookie`（保存期限 365 天）。
   - 具備自動容錯回退機制（無痕模式或限制 LocalStorage 時自動讀取 Cookie），零網路請求、零追蹤、零上傳，保障個人隱私。
